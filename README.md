...
=======
# TrafficShield
A cybersecurity project that analyzes network traffic from PCAP files and visualizes insights through a Python-Flask backend and interactive web dashboard.
...
Built on top of a C++ DPI engine, extended with a Python backend and a browser-based security dashboard.

---

##  Project Overview

Modern networks carry traffic from hundreds of applications simultaneously. This project answers the question:

> *"What is actually flowing through a network, and is any of it a threat?"*

The system reads a `.pcap` file (network capture), inspects each packet deep into its payload, identifies which application generated it (YouTube, Facebook, DNS, etc.), and presents everything visually — blocked packets, top domains, protocol breakdown, and more.
...
.pcap file
    ...
    ▼
┌─────────────────────────┐
│   C++ DPI Engine        │  ← Parses packets, extracts TLS SNI,
│   (dpi_engine/)         │    classifies apps, applies block rules
└────────────┬────────────┘
             │ stdout report
             ▼
┌─────────────────────────┐
│   Python Parser         │  ← Converts DPI output → structured JSON
│   (analyzer/parse.py)   │
└────────────┬────────────┘
             │ report.json
             ▼
┌─────────────────────────┐
│   Flask Backend         │  ← Serves data via REST API
│   (backend/app.py)      │
└────────────┬────────────┘
             │ GET /api/report
             ▼
┌─────────────────────────┐
│   Web Dashboard         │  ← Interactive charts & tables in browser
│   (dashboard/index.html)│
└─────────────────────────┘
>>>>>>> a0c87e2235ca401213fb0d2ccea508cb2316e064
```
...
-  **Multi-threaded Engine** — parallel processing for high-volume captures
...
│
├── dpi_engine/                 # C++ DPI engine (core packet inspector)
│   ├── include/                # Header files
│   ├── src/                    # Source files
│   └── CMakeLists.txt
│
├── analyzer/
│   └── parse_output.py         # Parses DPI text output → JSON
│
├── backend/
...
│
├── sample_data/
│   └── test_dpi.pcap           # Sample network capture for demo
│
...
=======
> On Windows, refer to `WINDOWS_SETUP.md` inside `dpi_engine/`.
...
```bash
# Basic usage (no blocking)
python analyzer/parse_output.py sample_data/test_dpi.pcap
...
This runs the DPI engine and saves `report.json`.
...
| **Blocked vs Forwarded** | Bar chart — how many packets passed/dropped |
| **Top Domains** | Table of detected SNI hostnames |
| **Protocol Split** | TCP vs UDP vs DNS |
| **Alert Log** | List of blocked connections with reason |

---

##  How DPI Works (Key Concepts)

### The Five-Tuple
Every network connection is uniquely identified by:
`(Source IP, Destination IP, Source Port, Destination Port, Protocol)`

All packets sharing the same five-tuple belong to one connection (flow).

### SNI Extraction
Even HTTPS traffic is partially visible. The **TLS Client Hello** message — sent before encryption kicks in — contains the destination domain in plaintext as the **Server Name Indication (SNI)**:
...
### Flow-Based Blocking
Once a flow is identified (e.g., YouTube), **all** subsequent packets of that connection are dropped — not just the one that revealed the SNI.
...
- Add **real-time capture** using `libpcap` instead of reading from file
- Add **GeoIP lookup** to show where traffic is going on a world map
- Add **anomaly detection** — flag unusual traffic spikes
- Add **HTTP host extraction** for unencrypted traffic
